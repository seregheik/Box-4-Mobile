...
import { View, StyleSheet, ScrollView, TouchableOpacity, useColorScheme, ActivityIndicator, Image, Alert } from 'react-native';
...
export function Step3Photos() {
  const { formData, photos, setPhotos, prevStep, resetForm } = useAddListingStore();
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity 
        style={[styles.uploadButton, { borderColor: theme.tintBlue, backgroundColor: theme.backgroundElement }]} 
        onPress={pickImage}
      >
        <Ionicons name="camera-outline" size={32} color={theme.tintBlue} />
        <ThemedText style={{ color: theme.tintBlue, marginTop: Spacing.two }}>Upload Photos</ThemedText>
      </TouchableOpacity>
      ...
      <View style={styles.photosGrid}>
        {photos.map((uri, index) => (
          <View key={index} style={styles.photoContainer}>
            ...
            >
              <Ionicons name="close-circle" size={24} color={Colors.light.tintRed} />
            </TouchableOpacity>
          </View>
        ))}
      </View>
